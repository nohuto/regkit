...
void ConfigureReadOnlyNameField(HWND dlg, const std::wstring& name) {
  ...
  ApplyThinEditBorder(dlg, IDC_VALUE_NAME);
}
...
INT_PTR CALLBACK BinaryDialogProc(HWND dlg, UINT msg, WPARAM wparam, LPARAM lparam) {
  ...
  case WM_INITDIALOG: {
    ...
    ConfigureValueDetails(dlg, state->value_name, state->value_type, state->show_details, false, 20, {IDC_LABEL, IDC_EDIT, IDC_NOTE, IDC_BINARY_PREVIEW, IDC_FORMAT_GROUP, IDC_FORMAT_BYTE, IDC_FORMAT_WORD, IDC_FORMAT_DWORD, IDC_FORMAT_QWORD, IDC_TEXT_GROUP, IDC_TEXT_ANSI, IDC_TEXT_UNICODE, IDOK, IDCANCEL});
    SetDlgItemTextW(dlg, IDC_EDIT, state->text.c_str());
    ApplyThinEditBorder(dlg, IDC_EDIT);
